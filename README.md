# Muhammad Asif 👋  
**Aspiring AI Engineer | BSCS Student | Python Developer**

📍 Peshawar, Khyber Pakhtunkhwa, Pakistan  

---

## 👨‍💻 About Me

Hi, I’m **Muhammad Asif**, a passionate **Computer Science student** and future **AI Engineer**.  
I am currently in my **7th semester of BSCS** at the **University of Peshawar** and actively building real-world AI systems.

I am enrolled in advanced AI programs and focused on **Agentic AI**, **LLMs**, and **Python-based automation**.  
I love creating intelligent systems that can **reason, automate tasks, and solve real problems**.

---

## 🎓 Education

- **BS Computer Science**  
  *University of Peshawar* (2022 – 2026)

- **Certified Cloud Applied Generative AI Engineer**  
  *PIAIC* (2024 – 2025)

- **Secondary School Examination**  
  *Army Public School (APSACS)*

---

## 🧠 Technical Skills

- **Programming:** Python (Advanced), OOP  
- **AI & LLMs:** OpenAI API, LangChain, Agentic AI  
- **Frameworks & Tools:** Streamlit, Chainlit, HuggingFace  
- **Databases & Search:** FAISS  
- **Version Control:** Git & GitHub  

---

## 🚀 What I’m Working On

- 🤖 AI Chatbots & Virtual Assistants  
- 🧠 Autonomous AI Agents  
- ⚙️ Task Automation using LLMs  
- 📊 Real-time Python & AI applications  

---

## Certifications

- Introduction to Generative AI  
- AI Python for Beginners  
- Introduction to LangGraph  
- Introduction to Image Generation  
- Modern AI Python Programming (PIAIC)

---

## Goals

- Become a professional **AI Engineer**
- Build scalable **Agentic AI systems**
- Contribute to **open-source AI projects**
- Collaborate with developers worldwide

---

## 🤝 Let’s Connect

I’m always open to learning, collaboration, and exciting AI projects.

📧 Email: asifnasaaz2005@gmail.com  
🔗 LinkedIn: https://www.linkedin.com/in/muhammadasif2005  


